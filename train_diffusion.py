...
from DIT.dataset import MNIST
from DIT.add_noise import forward_add_noise
from DIT.config_dit import T


DEVICE='cuda' if torch.cuda.is_available() else 'cpu'

dataset = MNIST()
model=DiT(img_size=28,patch_size=4,in_channel=1,emb_size=128,label_num=10,dit_num=8,head=8).to(DEVICE) # 模型
...
for epoch in tqdm(range(EPOCH)):
    for imgs,labels in dataloader:
        ...
        if iter_count%1000==0:
            ...
            os.replace('.dit_model.pth','./checkpoints/dit/model.pth')
        iter_count+=1

...
class Emtn(nn.Module):
    def __init__(self, local_rank):
        ...
        self.rotation_net =  SixDRepNet_Detector(local_rank, dict_path='checkpoints/6DRepNet_300W_LP_AFLW2000.pth')
...
class Gbase(nn.Module):
    def __init__(self, is_train=False, local_rank=0):
        super(Gbase, self).__init__()
        self.is_train = is_train
        self.appearanceEncoder = Eapp()
        self.motionEncoder = Emtn(local_rank)
        ...
        for i, layer in enumerate(model.children()):
            ...
            if i in layers:
                if detach:
                    ...
                    tmp = torch.mean(torch.abs(predicted_features - target_features)+ 1e-6 )
                    # print(tmp)
                    # quit()
                    # if tmp.isnan():    
                    #     loss += 1e-6
...
class GANLoss(nn.Module):
    ...
    def __call__(self, input, target_is_real, local_rank):
        if isinstance(input[0], list):
            loss = 0
            for input_i in input:
                pred = input_i[-1]
                target_tensor = self.get_target_tensor(pred, target_is_real).to(local_rank)
                loss += self.loss(pred, target_tensor)
            return loss
        else:            
            target_tensor = self.get_target_tensor(input[-1], target_is_real).to(local_rank)

...
def cosine_loss(pos_pairs, neg_pairs,  s=5.0,m=0.2, device_id=0): # We also set 𝑠 = 5 and 𝑚 = 0.2 in the cosine loss.
    loss = torch.tensor(0.0, requires_grad=True).to(device_id)

    for pos_pair in pos_pairs:
        pos_sim = F.cosine_similarity(pos_pair[0], pos_pair[1])
        neg_loss = torch.tensor(0.0, requires_grad=True).to(device_id)
...
def train_base(cfg, Gbase, Dbase, dataloader, local_rank):
    ...
    for epoch in range(begin_epoch, cfg.training.base_epochs):
        ...
        # while batch is not None:
            ...
            loss_cos = cosine_loss(pos_pair, neg_pair, device_id=local_rank)
            # These losses are from the paper of vasa
            # dynamics_transfer_loss = mse_loss(pose2drive, dyn2source)
            # cosine_similar_loss = cosine_feat_loss(output_frame, output_frame_random)
            ...
            # print('local rank:', local_rank, 'debug backward')
            ...
            # Train discriminator
            # print('local rank:', local_rank,'debug14')
            ...
            # print('local rank:', local_rank,'debug15')
            ...
            if cur_step % 1 == 0 and dist.get_rank() == 0:
...
def main(cfg: OmegaConf) -> None:
  ...
  #     transforms.RandomHorizontalFlip(),
   #     transforms.ColorJitter() # "as augmentation for both source and target images, we use color jitter and random flip"
    ...
    Dbase = model.Discriminator(input_nc=3).to(device_id) # 🤷

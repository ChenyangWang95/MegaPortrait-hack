    ...
    for epoch in range(begin_epoch, total_epoch):
        ...
        for batch in dataloader:
            ...
            with autocast():
                ...
                vgg19_loss, vggface_loss, lpips_loss = perceptual_loss_fn(output_frame_mask, driving_frame_mask)
                ...
                total_loss = cfg.training.w_per * vgg19_loss + \
                            ...
                            # cfg.training.w_fm * loss_fm + \ cfg.training.w_lpips * lpips_loss + \
                ...
                if cur_step % 100 == 0 and dist.get_rank() == 0:
                    ...
                    vutils.save_image(img_save, f"output_images/all_loss_fg_celeb2_lpips/{cur_step}.png")
                if dist.get_rank() == 0:
                    wandb.log({"Loss_G": total_loss.item(),
                            ...
                            step=cur_step)
                del total_loss, s_start_d_pred, output_frame_mask, output_frame, z_s_start_d, z_s_d, z_d, z_d_star
        ...
        if (epoch + 1) % cfg.training.save_interval == 0 and dist.get_rank() == 0:
            torch.save(Gbase.module.state_dict(), f"checkpoints/Gbase/Gbase_lpips_epoch{epoch+1}.pth")
            torch.save(Dbase.module.state_dict(), f"checkpoints/Dbase/Dbase_lpips_epoch{epoch+1}.pth")
...
def main(cfg: OmegaConf) -> None:
    ...
    if dist.get_rank() ==0:
        wandb.init(project="Disentanglement-ddp", entity="marvin_tec", name="all_loss_fg_celeb2_lpips", config=config, settings=wandb.Settings(start_method="fork"))

...
from frames_dataset_images import FramesDataset
    ...
    for epoch in range(begin_epoch, total_epoch):
        ...
        for batch in dataloader:
            ...
            with autocast():
                ...
                output_frame, I_j_hat_i_pose, I_i_hat_j_dyn = Gbase(source_frame, driving_frame, same_subject=True)
                ...
                loss_discrepancy = discrepancy_loss(I_j_hat_i_pose, I_i_hat_j_dyn)
                ...
                total_loss = cfg.training.w_per * vgg19_loss + \
                            ...
                            cfg.training.w_discrepancy * loss_discrepancy
                ...
                if cur_step % 5 ==0 and dist.get_rank() == 0:
                    ...
                    logging.info(f"Epoch [{epoch+1}/{total_epoch}], "
                                ...
                                f"Loss_discrepancy: {loss_discrepancy.item():.4f}, "
                    ...
                    wandb.log({"Loss_G": total_loss.item(),
                                ...
                                "Loss_dyn_transfer": loss_discrepancy.item(),
                ...
                if cur_step % 50 == 0 and dist.get_rank() == 0:
                    img_save = torch.cat([source_frame, random_source_frame, driving_frame, output_frame, s_start_d_pred, I_j_hat_i_pose, I_i_hat_j_dyn], dim=-2)

                    vutils.save_image(img_save, f"output_images/withbg_hrdata_refine_gan_crossloss/{cur_step}.png")
                    del img_save
...
def main(cfg: OmegaConf) -> None:
    ...
    if dist.get_rank() ==0:
        wandb.init(project="Disentanglement-ddp", entity="marvin_tec", name="gaze_refined_MultiScaleD_crossloss_v2", config=config, settings=wandb.Settings(start_method="fork"))

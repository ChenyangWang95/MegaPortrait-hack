    ...
    for epoch in range(begin_epoch, total_epoch):
        ...
        for batch in dataloader:
            ...
            with autocast():
                ...
                # real loss
                real_pred = Dbase(driving_frame_mask, source_frame)
                fake_pred = Dbase(output_frame_mask.detach(), source_frame)
                ...
                # loss_D_fake = d_loss(fake_pred, fake)
                # loss_D_real = d_loss(real_pred, valid)
                ...
                if cur_step % 10 == 0 and dist.get_rank() == 0:
                    img_save = torch.cat([source_frame, driving_frame, random_driving_frame, output_frame, s_start_d_pred], dim=2)
                    vutils.save_image(img_save, f"output_images/train_dataset_cross/output_imgs/{cur_step}.png")
                if dist.get_rank() == 0:
                    wandb.log({"Loss_G": total_loss.item(),
                                "Loss_fm": loss_fm.item(),
                                "Loss_per": loss_G_per.item(),
        ...
        if (epoch + 1) % cfg.training.save_interval == 0 and dist.get_rank() == 0:
            torch.save(Gbase.module.state_dict(), f"checkpoints/Gbase/Gbase_2_epoch{epoch+1}.pth")
            torch.save(Dbase.module.state_dict(), f"checkpoints/Dbase/Dbase_2_epoch{epoch+1}.pth")
...
def main(cfg: OmegaConf) -> None:
    ...
    parser.add_argument('--Gbase_path', default='checkpoints/Gbase/Gbase_2_epoch1.pth', type=str, help='resume Gbase ckpt')
    parser.add_argument('--Dbase_path', default='checkpoints/Dbase/Dbase_2_epoch1.pth', type=str, help='resume Dbase ckpt')
    ...
    if dist.get_rank() ==0:
        wandb.init(project="Disentanglement-ddp", entity="marvin_tec", name="all_loss_fg", config=config, settings=wandb.Settings(start_method="fork"))
